...
public class Minigames {
    //IDs
    public static int mainInterfaceID = 76, clanInterfaceID = 589;
    ...
    public static boolean selectMinigame(MINIGAMES name) {
        //Selects Desired Minigame. Returns false if failed to select.
        if (!isMinigameTabOpen()) {
            //Can be used to check if desired Minigame is already selected.
            openMinigameTab();
        }
        return selectMinigame(name.getName());
    }

    public static boolean isMinigameTabOpen() {
        //Checks if Minigame Tab is open. Returns false if not.
    ...
    public static boolean inMinigameChat() {
        //CHECKS IF YOU ARE IN A MINIGAME CHAT. Returns false if not.
    ...
    public static boolean openMinigameTab() {
        //Opens the Minigame tab inside the Quests tab. Returns false if failed to open.
    ...
    public static boolean joinChat() {

        //Joins the chat of selected minigame. Returns false if failed to join.
        if (!isMinigameTabOpen()) {
            openMinigameTab();
        }
    ...
    public static boolean teleport() {
        //Teleports with selected minigame. Returns false if Teleport failed.
        if (!isMinigameTabOpen()) {
            openMinigameTab();
        }
        RSInterfaceMaster mini = Interfaces.get(mainInterfaceID);
        if (mini != null) {
            ...
            if (button != null) {
                ...
                }, 4000)) return Timing.waitCondition(new Condition() {
                    @Override
                    public boolean active() {
                        General.sleep(350);
    ...
    private static boolean isSelected(String name) {
        ...
        String minigame;
        return (mini != null && (child = mini.getChild(11)) != null && (minigame = child.getText()) != null && minigame.toLowerCase().contains(name.toLowerCase()));
    }
    ...
    private static boolean selectMinigame(String name) {
        ...
        if (isSelected(name)) {
            ...
            if (MINI != null && (child = MINI.getChild(11)) != null) {
                Clicking.click(child);
                if (Timing.waitCondition(new Condition() {
                    @Override
                    public boolean active() {
                        General.sleep(350);
                        RSInterfaceChild minigamesBox = MINI.getChild(21);
                    ...
                    if (minigamesBox != null) {
                        ...
                        if (miniRec != null) {
                            if (!rec.contains(miniRec)) {
                                ...
                                Point randomized = new Point((int) scroll.getX() + General.random(1, scroll.width - 1), (int) scroll.getY() + General.random(1, scroll.height - 1));
